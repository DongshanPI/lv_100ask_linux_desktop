...
static void lv_task_wlan_scan(lv_timer_t * task)
{
    ...
    {
        //printf("[wlan_scan_thread_func]res_buffer: %s\n", res_buffer);
...
{
    if(shell_exec_handle("ifconfig wlan0 up", "ERROR") != 1)
        return 0;
    ...
    {
        lv_timer_resume(g_wifi_scan_task_handle);
    }
    lv_timer_ready(g_wifi_scan_task_handle);  // 就绪，马上开始
...
/*
    ...
    只获取ip地址： ifconfig wlan0 | grep "inet " | awk '{print $2}'
...
static void conn_btn_event_handler(lv_event_t * e)
{
    ...
    char * wifi_name = lv_event_get_user_data(e);   // 获取在对象上注册事件时传递的 user_data
...
static void wifi_conn_info(char * wifi_name)
{
    ...
    lv_obj_set_size(input_panel, 280, 75);
...
void imx6ull_set_wlan_init(void)
{
    ...
    lv_obj_set_size(g_wifi_scan_list, LV_PCT(100), LV_PCT(70));
